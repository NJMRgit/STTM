...
class BlurSwitchManager(QMainWindow):
    ...
    def _generate_colors_from_wallpaper(self, mode, auto=False):
        ...
        dlg.setStyleSheet(MSG_BOX_STYLE + " QDialog { background: rgba(20,20,20,0.2); } QLabel { color: white; font-size: 13px; }")
        dlg.setWindowFlags(Qt.WindowType.Dialog)
        dlg.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)
        layout = QVBoxLayout(dlg)
        layout.setContentsMargins(20, 20, 20, 20)
        layout.setSpacing(12)
        label = QLabel("<p style='text-align:center;'>How would you like to generate colors?</p>")
        label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(label)
        btn_style = "QPushButton { background-color: rgba(128,128,128,0.3); color: white; font-weight: bold; padding: 6px 16px; border: 1px solid rgba(128,128,128,0.4); border-radius: 4px; min-width: 200px; } QPushButton:hover { background-color: rgba(128,128,128,0.5); }"
        btn_wp = QPushButton("Generate from Wallpaper")
        btn_wp.setStyleSheet(btn_style)
    ...
    def _stop_auto(self):
        try:
            ...
            if self._auto_proc is not None:
                ...
                if not self._auto_proc.waitForFinished(3000):
                    self._auto_proc.kill()
                    self._auto_proc.waitForFinished(1000)
                self._auto_proc.deleteLater()
                self._auto_proc = None
